...
<div class="meditaj-registration-container">
	...
	<?php else : ?>
		...
		<form method="post" enctype="multipart/form-data" class="meditaj-register-form" novalidate>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				...
				const registerForm = document.querySelector('.meditaj-register-form');
				if (registerForm) {
					registerForm.addEventListener('submit', function(e) {
						...
						requiredFields.forEach(function(field) {
							...
							if (el && !el.value.trim()) {
								errors.push(field.label + ' ' + '<?php echo esc_js( __( 'is required.', 'meditaj' ) ); ?>');
						...
						if (inputAccType.value === 'bank') {
							...
							bankFields.forEach(function(field) {
								const el = document.getElementById(field.id);
								if (el && !el.value.trim()) {
									errors.push(field.label + ' ' + '<?php echo esc_js( __( 'is required for bank payout.', 'meditaj' ) ); ?>');
									el.style.border = '1px solid #ef4444';
								} else if (el) {
									el.style.border = '';
								}
							...
							mobileFields.forEach(function(field) {
								const el = document.getElementById(field.id);
								if (el && !el.value.trim()) {
									errors.push(field.label + ' ' + '<?php echo esc_js( __( 'is required for mobile payout.', 'meditaj' ) ); ?>');
									el.style.border = '1px solid #ef4444';
								} else if (el) {
									el.style.border = '';
								}
						...
						if (photoInput && photoInput.files.length > 0) {
							if (photoInput.files[0].size > maxFileSize) {
								errors.push('<?php echo esc_js( __( 'Profile photo file size exceeds 2MB limit.', 'meditaj' ) ); ?>');
						...
						if (certInput && certInput.files.length > 0) {
							if (certInput.files[0].size > maxFileSize) {
								errors.push('<?php echo esc_js( __( 'BMDC/Medical Certificate file size exceeds 2MB limit.', 'meditaj' ) ); ?>');
								certInput.style.outline = '1px solid #ef4444';
							} else {
								certInput.style.outline = '';
							}
						} else {
							errors.push('<?php echo esc_js( __( 'BMDC/Medical Certificate is required.', 'meditaj' ) ); ?>');
						}
						...
						if (errors.length > 0) {
							...
							const existingBox = document.getElementById('meditaj-frontend-errors');
							if (existingBox) {
								existingBox.remove();
							}
							...
							alertDiv.id = 'meditaj-frontend-errors';
							alertDiv.className = 'meditaj-alert meditaj-alert-error';
							...
							alertDiv.innerHTML = `
								...
								<div class="meditaj-alert-content">
									<strong>${'<?php echo esc_js( __( 'Form Validation Errors:', 'meditaj' ) ); ?>'}</strong>
							...
							const submitWrap = document.querySelector('.meditaj-submit-wrap');
							if (submitWrap) {
